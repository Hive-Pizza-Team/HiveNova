...
class Session
{
	...
	if(!(isset($_GET['page']) && $_GET['page']=="raport" && isset($_GET['raport']) && count($_GET)==2 && MODE === 'INGAME')) {
		...
		if(empty($created)) {
			...
			$session_is_exists = $db->selectSingle($sql_is_exists, array(':sessionId' => session_id()));
			if(empty($session_is_exists)) {
			$sql_forcekill = 'REPLACE INTO uni1_session_forcekill SET
			sessionID	= :sessionId,
			 created	= :created;';

			$db->replace($sql_forcekill, array(
			':sessionId'	=> session_id(),
			':created'	=> $created,
			)); }
		}
		...
		$sql_force_kill_data = $db->selectSingle($sql_force_kill, array(
			':sessionId'	=> session_id(),
