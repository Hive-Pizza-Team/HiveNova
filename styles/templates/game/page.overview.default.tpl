...
{if $Moon}<div class="moon overview-planet-moon"><a href="game.php?page=overview&amp;cp={$Moon.id}&amp;re=0" title="{$Moon.name}">{include file="shared.planet-thumb.tpl" texture='mond' dpath=$dpath width=100 height=100 alt="{$Moon.name} ({$LNG.fcm_moon})" preferLite=true loading="lazy"}</a><br>{$Moon.name} ({$LNG.fcm_moon})
</div>
{/if}
	<div class="overview-planet-main">
		...
		<div class="planeth overview-planet-visual overview-planet-visual--fallback">
			<img class="overview-planet-fallback" src="{$dpath}planeten/{$planetimage}.jpg" onerror="this.onerror=null;this.src='{$dpath}planeten/{$planetimage}_hq.jpg'" alt="{$planetname}">
		</div>
		{/if}
...
{if $AllPlanets}<div class="planeto">{$LNG.lv_planet}</div>
			...
			<div class="planetl"><a href="game.php?page=overview&amp;cp={$PlanetRow.id}" title="{$PlanetRow.name}">{include file="shared.planet-thumb.tpl" texture=$PlanetRow.image dpath=$dpath width=100 height=100 style='margin: 5px;' loading='lazy' preferLite=true alt=$PlanetRow.name}</a><br>{$PlanetRow.name}<br>{$PlanetRow.build|default:''}<br></div>
