...
<tr>
	...
	<td><a href="#" data-tooltip-content="<table style='width:200px'>{foreach $FleetRow.resource as $resourceID => $resourceCount}<tr><td style='width:50%'>{$LNG.tech.$resourceID}</td><td style='width:50%'>{$resourceCount|number_plain}</td></tr>{/foreach}</table>" class="tooltip">{$LNG["type_mission_{$FleetRow.missionID}"]}{if $FleetRow.acsID != 0}<br>{$FleetRow.acsID}<br>{$FleetRow.acsName}{/if}&nbsp;(R)</a></td>
	<td>{$FleetRow.starttime}</td>
	<td><a href="#" data-tooltip-content="<table style='width:200px'>{foreach $FleetRow.ships as $shipID => $shipCount}<tr><td style='width:50%'>{$LNG.tech.$shipID}</td><td style='width:50%'>{$shipCount|number_plain}</td></tr>{/foreach}</table>" class="tooltip">{$FleetRow.count|number}&nbsp;(D)</a></td>
    ...
    <td>
		<form method="post" action="admin.php?page=fleets" style="display:inline">
			<input type="hidden" name="id" value="{$FleetRow.fleetID}">
			{if $FleetRow.lock}
			<input type="hidden" name="lock" value="0">
			<button type="submit" style="color:lime;background:none;border:0;padding:0;cursor:pointer">{$LNG.ff_unlock}</button>
			{elseif $FleetRow.error}
			<input type="hidden" name="lock" value="2">
			<button type="submit" style="color:red;background:none;border:0;padding:0;cursor:pointer">{$LNG.ff_del}</button>
			{else}
			<input type="hidden" name="lock" value="1">
			<button type="submit" style="color:red;background:none;border:0;padding:0;cursor:pointer">{$LNG.ff_lock}</button>
			{/if}
		</form>
	</td>
</tr>
