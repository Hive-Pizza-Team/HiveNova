...
class ShowOverviewPage extends AbstractGamePage
{
	...
	private function GetTeamspeakData()
	{
		...
		Cache::get()->add('teamspeak', 'HiveNova\\Core\\Cache\\TeamspeakBuildCache');
	...
	function delete() 
	{
		...
		if ($USER['b_tech_planet'] == $PLANET['id'] && !empty($USER['b_tech_queue'])) {
			...
			if ($IfFleets > 0) {
			...
			} elseif ($USER['id_planet'] == $PLANET['id'] && $currentPlanetCount == 1) {
                ...
                if($PLANET['planet_type'] == 1) {
					...
					if ($USER['id_planet'] == $PLANET['id']) {
						// set id_planet to next non-destroyed planet
						$sql	= "SELECT `id` FROM %%PLANETS%%
									WHERE `id_owner` = :userId
									AND `id` != :planetId
									AND `destruyed` = :destroyed
									LIMIT 1;";
						$NEXT_PLANET	= $db->selectSingle($sql, array(
							':userId'	=> $USER['id'],
							':planetId'	=> $PLANET['id'],
							':destroyed'=> 0
						));
						...
						$db->update($sql, array(
							':new_main_planet_id'   => $NEXT_PLANET['id'],
							':userId' => $USER['id'],
						));
						// refresh the page
