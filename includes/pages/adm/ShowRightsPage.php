	...
	switch($mode)
	{
		case 'rights':
			...
			if ($_POST)
			{
				...
				if($_POST['action'] == 'send') {
					Database::get()->update(
						...
						[':rights' => serialize(array_map(intval(...), $_POST['rights'])), ':id' => $id]
					);
				}

				$Rights	= Database::get()->selectSingle(
			...
			if ($authlevelFilter !== null) {
				$rows = Database::get()->select(
					"SELECT `id`, `username`, `authlevel` FROM %%USERS%% WHERE `universe` = :universe AND `authlevel` = :authlevel;",
					[':universe' => Universe::getEmulated(), ':authlevel' => $authlevelFilter]
				);
			} else {
				$rows = Database::get()->select(
					"SELECT `id`, `username`, `authlevel` FROM %%USERS%% WHERE `universe` = :universe;",
		...
		case 'users':
			...
			if ($_POST)
			{
				...
				Database::get()->update(
					"UPDATE %%USERS%% SET `authlevel` = :authlevel WHERE `id` = :id;",
					[':authlevel' => $authlevel, ':id' => $id]
				);
